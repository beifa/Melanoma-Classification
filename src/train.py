...
PATH_LOG = r'C:\Users\pka\kaggle\melanoma\log'
...
if __name__ == "__main__":
    ...
    if pars.debag:
        print('Debag....')
        print(pars)
        df = pd.read_csv(os.path.join(PATH, 'train_folds.csv')).head(10000)
    else:
        df = pd.read_csv(os.path.join(PATH, 'train_folds.csv'))
    tr_idx = np.where(df.fold != params['fold'])
    vl_idx = np.where(df.fold == params['fold'])
    td = trainDataset(df.loc[tr_idx], PATH_PNG_224, transform= transforms_train)
